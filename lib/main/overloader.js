...
module.exports = function(h_handler) {
	...
	// also implement/override with custom methods
	if(h_handler.custom) {

		// ref custom section
		let h_custom = h_handler.custom;

		// each key in custom section
		for(let s_key in h_custom) {

			// set on interface
			f_interface[s_key] = h_custom[s_key];
		}
	}
