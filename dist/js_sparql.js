 ...
 **/
	...
	var hash_gpd = function hash_gpd(h_source, f_update) {
	...
	var rdf_thing = function rdf_thing(s_thing) {
		if (H_SPARQL_CHARS[s_thing[0]]) return s_thing;else if (s_thing.indexOf(':') != -1) return s_thing;else if (s_thing == 'a') return s_thing;else return ':' + s_thing;
	};

	// converts variable alias to proper identifier if not already specified
	var rdf_var = function rdf_var(s_var) {
		if (H_SPARQL_CHARS[s_var[0]]) return s_var;else if (s_var.indexOf(':') != -1) return s_var;else return '?' + s_var;
	};

	// converts string to proper identifier if not already specified
	var rdf_lit = function rdf_lit(s_str) {
		if (H_SPARQL_CHARS[s_str[0]]) return s_str;else if (s_str.indexOf(':') != -1) return s_str;else if (/^(?:[0-9]|(?:\-|\.|\-\.)[0-9])/.test(s_str)) return s_str;else return JSON.stringify(s_str);
	};

	// converts query hash to sparql string
	var to_sparql = function to_sparql(sq_prefixes) {
  ...
  **/
		...
		var submit_query = function submit_query(s_query, f_okay) {
			$.ajax({
				...
				dataType: 'json',
				success: function success(h_res) {
					f_okay && f_okay(h_res);
				}
			});
		};
		...
		var qb = function qb(h_query) {
			...
			var d_self = {
				select: function select(z_select, b_reset) {
				...
				exclude: function exclude(s_vars) {
				...
				join_lists: function join_lists(h_items, s_delim) {
					var a_exclude = [];
				...
				each: function each(f_each, f_okay) {
					f_exec(function (h_res) {
						if (!f_each) return;
						var a_rows = h_res.results.bindings;
						for (var i = 0, l = a_rows.length; i < l; i++) {
		...
		var build_from_ph = function build_from_ph(h_predicates, _) {
			...
			for (var s_rp in h_predicates) {
				...
				// not-first predicate
				if (b_nfp) s_out += ' ;';else b_nfp = true;
				...
				// number literal
					...
					// function
						...
						// regex literal
								...
								var s_flags = '' + (z_object.global ? 'g' : '') + (z_object.ignoreCase ? 'i' : '') + (z_object.multiline ? 'm' : '');
		...
		var query_hash = function query_hash(h_subjects, b_raw) {
		...
		var query_string = function query_string(s_query) {
  ...
  * public operator() ();
  **/
		var operator = function operator(z_query) {
  ...
  **/
		...
		operator['endpoint'] = function (s_url) {
			if (!arguments.length) return s_endpoint_url;else s_endpoint_url = s_url;
		};
 ...
 **/
	...
	(function () {
		...
		var debug = function debug(channel) {
			return function () {
	...
	(function () {
		var global_instance = function global_instance(s_method) {
			return function () {
