...
const H_PATTERNS = {
	...
	filter: (add, h_gp) => {
		...
		let b_pattern = ('object' === typeof z_expression && -1 !== ['exists', 'not exists'].indexOf(z_expression.operator));
...
* class:
**/
const local = classer('QueryBuilder', function(h_init) {
	...
	**/
	...
	// distinct, reduce
	let b_select_distinct = false;
	let b_select_reduced = false;
			...
			if(a_values_combos.length) {
				...
				a_values_combos.forEach((h_combo) => {
					...
					for(let s_key in h_combo) {
						...
						if(-1 === a_values_variables.indexOf(s_key)) {
	...
	// select query
		...
		k_query_producer.set('query', function(add) {
			...
			// yes variables
				...
				// distinct
				if(b_select_distinct) {
					add(`distinct`, true);
				}
				// reduced
				else if(b_select_reduced) {
					add(`reduced`, true);
				}
	...
	// insert-data query
		...
		k_query_producer.set('query', function(add) {
			...
			for(let s_graph_iri in h_into_graphs) {
				...
				//
				a_graph_triples.forEach((z_triple) => {

					// triple is string
					if('string' === typeof z_triple) {
						add(z_triple);
					}
					else {
						// add triple
		...
		let insert_data_query = class extends basic_query {
			...
			data(...a_ttls) {
				...
				a_ttls.forEach((z_ttl) => {
					...
					// triple is string
					else if('string' === typeof z_ttl) {
						a_graph_triples.push(z_ttl);
					}
					// 
					else {
