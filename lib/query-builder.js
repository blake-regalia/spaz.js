	...
	**/
	...
	const k_query_producer = rapunzel({
		...
		where: (add) => {
			...
			// simple variable => values mapping exists
			if(hm_values_data.size) {

				// each variable mapping
				for(let [s_var, as_values] of hm_values_data) {

					// open simple values block
					add.open('values '+s_var+(add.pretty? ' ': '')+'{');

					// iterate the set
					for(let s_value of as_values) {
	...
	//
	const add_values_data = (z_block, a_vars) => {
		...
		// key/value map
			...
			for(let s_var of a_vars) {
				...
				if(hm_values_data.has(s_var)) {

					// ref existing set
					as_data = hm_values_data.get(s_var);
				}
				// map does not have this key yet
					...
					// store it in the map
					hm_values_data.set(s_var, as_data);
				}
				...
				// data type is invalid
					...
					local.error('VALUES clause expects data to be [string]. instead got: '+arginfo(z_data));
				}
			}
		}
		// unexpected type
	...
	class basic_query {
		...
		// values clause
		values(...a_values) {
			local.fail('unable to produce values: '+arginfo(a_values));
		}
	...
	add_methods(basic_query, {
		...
		values: overloader({
			...
			reset: (a_values, a_vars) => {

				// clear values
				hm_values_data.clear();

				// apply each item in array
				a_values.forEach((z_value) => {
					add_values_data(z_value, a_vars);
				});
			},

			// simply convert map => hash
			fetch: () => {
				return map_to_hash(hm_values_data);
			},
		...
		return new (class extends basic_query {
			...
			rows(f_okay_rows) {
				...
				h_parent.submit(this.sparql(), 'table', (h_response) => {
					...
					// forward the boolean response value to callback listener
					f_okay_rows.call(this, h_response['results']['bindings'], h_response['head']['vars'], h_response);
				});
			}
	...
	// insert-data query
		...
		let insert_data_query = class extends basic_query {
			...
			data(...a_triples) {
				...
				//
				a_triples.forEach((a_triple) => {

					a_graph_triples.push(a_triple);
				});
