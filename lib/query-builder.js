...
//
let query_producer = require(__dirname+'/query-producer.js');
...
// 
const overloader = function(h_handler) {
...
const next_blanknode = () => {
	...
	return '_:_b'+(i_global_blanknode++);
};
...
const serialize_raw_triple = (a_triple) => {
	return {
		subject: a_triple[0],
		predicate: a_triple[1],
		object: a_triple[2],
	};
};

//
const serialize_triple = (a_triple) => {
...
const serialize_blanknode = (a_triple) => {
	...
	// create root triple
	var h_root_triple = serialize_raw_triple([a_triple[0], a_triple[1], s_blanknode_id]);
	...
	if('object' === typeof z_blanknode) {
		...
		for(let s_predicate in z_blanknode) {
			...
			a_results.push(
				serialize_triple([s_blanknode_id, s_predicate, z_blanknode[s_predicate]])
			);
		}
	}
	// unsupported type
...
const serialize_group_graph_pattern = (z_group) => {
	...
	if(Array.isArray(z_group)) {
		...
		return {
			type: 'bgp',
			triples: serialize_triple(z_group),
		};
	}
	// triple via SPARQL string
		...
		return {
			type: 'bgp',
			triples: serialize_triple(a_triple),
		};
	}
	// 
	else {
	...
	**/
	...
	const k_query_producer = new query_producer({
		...
		where: function(add) {
			...
			// pass query building torch to group pattern builders
			a_where_ggps.forEach((h_ggp) => {
				h_ggp.apply(this, arguments);
			});
		...
		//
		values: (add) => {
			if(h_values_data.size) {
				
			}
		},

	}, ['prefix', 'dataset', 'where', 'solution', 'values']);
	...
	class basic_query {
			...
			a_groups.forEach((z_group) => {
				...
				if(h_previous && h_previous.type === h_ggp.type) {
					...
					if(Array.isArray(h_ggp.triples)) {
						...
						h_previous.triples = h_previous.triples.concat(h_ggp.triples);
					}
					// patterns
					else if(h_ggp.patterns) {

						// append patterns onto existing previous group
						h_previous.patterns = h_previous.patterns.concat(h_ggp.patterns);
					}
					// uh-oh
					else {
		...
		toSparql() {
			...
			// produce query
			return k_query_producer.produce();
		}
			...
			select: function(add) {
				...
				a_select_variables.forEach((s_var) => {
					...
					if(h_select_expressions.has(s_var)) {
						add(`${h_select_expressions.get(s_var)} as ${s_var}`);
					}
					// plain variable
					else {
						add(`${s_var}`);
