...
import n3_document from './n3-document';
...
const setup_remote = (h_engine) => {
	...
	if(h_engine.http_methods) {
		...
		if(h_http_methods.table) {
			...
			if(-1 === A_VALID_TABLE_HTTP_METHODS.indexOf(h_http_methods.table)) {
		...
		if(h_http_methods.graph) {
			...
			// invalid http method for table-result queries
			if(-1 === A_VALID_GRAPH_HTTP_METHODS.indexOf(s_method)) {
		...
		if(h_http_methods.update) {
			...
			// invalid http method for table-result queries
			if(-1 === A_VALID_UPDATE_HTTP_METHODS.indexOf(s_method)) {
	...
	let k_engine = (s_sparql, s_type) => {
		...
		let s_url = h_engine.url;

		// get methods should have the query path appended to url
		if('get' === s_method.toLowerCase()) {
			s_url += s_path;
		}
...
* class:
**/
const local = classer('Spaz', function(h_config) {
	...
	**/
	...
	(() => {
		...
		if(h_config) {
			...
			// verbosity
			if(h_config.verbosity) {

				// ref verbosity
				let s_verbosity = h_config.verbosity;

				// no such verbosity level
				if(!H_LOG_LEVELS.hasOwnProperty(s_verbosity)) {
					return local.fail(`no such verbosity level "${s_verbosity}"`);
				}

				// set log level
				n_log_level = H_LOG_LEVELS[s_verbosity];
			}
				...
				if(h_config_engine.endpoint) {
					...
					if(h_config_engine.http_methods) {
						...
						if('string' === typeof z_http_methods) {
							...
							if(-1 !== ['get', 'post'].indexOf(s_method)) {
	...
	operator = extend(operator, {
		...
		silently: {
			drop: {
				graph: (s_graph, f_okay) => {
					operator.submit(`drop silent graph <${operator.expand(s_graph)}>`, 'update', f_okay || function(){});
				},
			},
		},
		...
		**/
		...
		ttl(...a_args) {
			...
			// create new n3 document
			let q_document = n3_document({
				parent: operator,
		...
		**/
		...
		val(z_value, s_type) {
			...
			if('undefined' === typeof s_type) {
				...
				// hash
				else if(Object === z_value.constructor) {
				...
				// Date
				else if(Date === z_value.constructor) {
					// set value from ISO serialization
					s_value = z_value.toISOString();

					// set type for xsd:dateTime
					s_type = P_XSD_NAMESPACE+'dateTime';
				}
				// unsupported type
				else {
			...
			//
			return '"'+s_value+'"^^'+operator.shorten(s_type);
		},

		// constructs a shortened
		n3v(z_value, s_type) {
		...
		rabbit(s_thing) {
			if('?' === s_thing[0]) {
				...
				return {
					type: 'literal',
					value: m_literal[1],
					datatype: m_literal[2]? m_literal[2].replace(/^<|>$/g, ''): P_XSD_NAMESPACE+'string',
				};
			}
			else {
				return {
